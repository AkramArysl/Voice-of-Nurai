...
import { apiRequest, config, formatReportCategory, type Report, type User } from "../api";
  ...
  useEffect(() => {
    apiRequest<{ reports: Report[] }>("/api/reports")
      ...
      .catch((reportsError: unknown) => {
        setError(reportsError instanceof Error ? reportsError.message : "Не удалось загрузить репорты");
      })
    ...
    <>
      <section className="border-b border-slate-200 bg-white">
        <div className="mx-auto max-w-7xl px-4 py-14">
          <div className="mx-auto max-w-4xl text-center">
            <p className="mb-3 text-sm font-semibold uppercase tracking-wide text-rose-700">Безопасность в один клик</p>
            <h1 className="leading-tight">
              ...
              <span className="mt-3 block text-2xl font-semibold text-slate-950 md:text-4xl">помогает быстро позвать на помощь</span>
            </h1>
            <p className="mt-5 max-w-2xl text-lg leading-8 text-slate-600">
              Отправляйте SOS близким, делитесь текущей геолокацией, сообщайте об опасных местах и получайте подсказки AI-помощника.
            </p>
          </div>

          <div className="mx-auto mt-10 max-w-5xl rounded-lg border border-slate-200 bg-slate-50 p-3 shadow-sm">
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              <a href={protectedHref("/sos")} className="group rounded-md border border-rose-200 bg-white p-5 text-center transition hover:-translate-y-0.5 hover:border-rose-300 hover:shadow-md">
                ...
                <span className="mt-3 block text-base font-semibold text-slate-950">SOS</span>
              </a>
              <a
                href={config.botUrl}
                target="_blank"
              ...
              >
                ...
                <span className="mt-3 block text-base font-semibold text-slate-950">AI Помощник</span>
              </a>
              <a
              ...
              >
                ...
                <span className="mt-3 block text-base font-semibold text-slate-950">Доверенные лица</span>
              </a>
              <a
              ...
              >
                ...
                <span className="mt-3 block text-base font-semibold text-slate-950">Сообщить</span>
              </a>
            </div>
          </div>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-10">
        <div className="mb-4 flex items-center gap-2">
          ...
          <h2 className="text-2xl font-bold text-slate-950">Карта безопасности</h2>
        </div>
        <LeafletMap lat={42.8746} lng={74.5698} height="360px" />
      </section>
      ...
      <section className="mx-auto max-w-7xl px-4 py-12">
        ...
        {error && <p className="rounded-md border border-rose-200 bg-rose-50 p-4 text-rose-700">{error}</p>}
        {!loading && !error && reports.length === 0 && (
          <Card className="p-6 text-slate-600">Пока нет репортов. Первый репорт появится здесь после публикации.</Card>
        )}

        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {reports.map((report) => (
            <Card key={report.id} className="overflow-hidden bg-white">
              {report.photo_url && <img src={report.photo_url} alt="Фото репорта" className="h-48 w-full object-cover" />}
              <div className="space-y-3 p-5">
                <div className="flex items-start justify-between gap-3">
                  <span className="rounded-full bg-slate-100 px-3 py-1 text-sm font-medium text-slate-700">{formatReportCategory(report.category)}</span>
                  <time className="text-xs text-slate-500">{new Date(report.created_at).toLocaleDateString("ru-RU")}</time>
                </div>
                {report.description && <p className="leading-7 text-slate-700">{report.description}</p>}
